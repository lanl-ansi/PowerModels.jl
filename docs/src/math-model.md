...
\mbox{variables: } & \nonumber \\
& I^g_k \;\; \forall k\in G \nonumber \\
& I^d_k \;\; \forall k\in L \label{var_load_current} \\
& V_i \;\; \forall i\in N \nonumber \\
...
& S^{d}_k = \Re(V_i (I^d_k)^*) + j \Im(V_i (I^d_k)^*)  \;\; \forall k \in L  \label{eq_complex_power_definition_load}\\
& v^l_i \leq |V_i| \leq v^u_i \;\; \forall i \in N \nonumber\\
& \sum_{\substack{k \in G_i}} I^g_k - \sum_{\substack{k \in L_i}} I^d_k - \sum_{\substack{k \in S_i}} Y^s_k V_i = \sum_{\substack{(i,j)\in E_i \cup E_i^R}} I_{ij} \;\; \forall i\in N  \label{eq_kcl_current} \\
...
- Eq. $\eqref{var_total_current}$ - implicit, substituted out before implementation
- Eq. $\eqref{eq_complex_power_definition_gen}$  - models active and reactive power range of a generator `constraint_gen`
- Eq. $\eqref{eq_complex_power_definition_load}$  - defines a fixed load  `constraint_gen`
- Eq. $\eqref{eq_kcl_current}$  - Kirchhoff's current law in current variables  `constraint_current_balance`
- Eq. $\eqref{eq_current_from}$  - branch from-side current,  expressions in `variable_current`
- Eq. $\eqref{eq_current_to}$  - branch to-side current, expressions in `variable_current`
