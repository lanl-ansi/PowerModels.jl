...
PowerModels.summary(network_data) # quick table-like summary
```

The `summary` function generates a table-like text summary of the network data, which is helpful in quickly assessing the values in a data or solution dictionary.
...
```
network_data = PowerModels.parse_file("nesta_case3_lmbd.m")
PowerModels.summary(network_data) # default per-unit form
PowerModels.make_mixed_units(network_data)
PowerModels.summary(network_data) # mixed units form
```
