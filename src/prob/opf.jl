...
""
function run_opf_tap_shift(file, model_type::Type, optimizer; kwargs...)
    return run_model(file, model_type, optimizer, build_opf_tap_shift; kwargs...)
end

""
function build_opf_tap_shift(pm::AbstractPowerModel)
    variable_voltage(pm)
    variable_generation(pm)

    variable_transformer_tap(pm)
    variable_transformer_shift(pm)
    variable_branch_flow(pm)
    variable_dcline_flow(pm)
    ...
    for i in ids(pm, :branch)
        constraint_ohms_tap_shift_yt_from(pm, i)
        constraint_ohms_tap_shift_yt_to(pm, i)
