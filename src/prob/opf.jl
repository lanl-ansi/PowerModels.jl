...
function run_mn_opf_strg(file, model_type::Type, optimizer; kwargs...)
    return run_model(file, model_type, optimizer, post_mn_opf_strg; multinetwork=true, kwargs...)
end

""
function post_mn_opf_strg(pm::AbstractPowerModel)
    for (n, network) in nws(pm)
        variable_voltage(pm, nw=n)
        variable_generation(pm, nw=n)
...
function run_opf_ptdf(file, model_type::Type, optimizer; full_inverse=false, kwargs...)
    if !full_inverse
        return run_model(file, model_type, optimizer, post_opf_ptdf; ref_extensions=[ref_add_connected_components!,ref_add_sm!], solution_builder=solution_opf_ptdf!, kwargs...)
    else
        return run_model(file, model_type, optimizer, post_opf_ptdf; ref_extensions=[ref_add_connected_components!,ref_add_sm_inv!], solution_builder=solution_opf_ptdf!, kwargs...)
    end
end

""
function post_opf_ptdf(pm::AbstractPowerModel)
    variable_generation(pm)
