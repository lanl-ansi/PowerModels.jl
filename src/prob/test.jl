...
"opf with tap magnitude and angle as optimisation variables"
function _run_opf_tap_shift(file, model_type::Type, optimizer; kwargs...)
    return run_model(file, model_type, optimizer, _build_opf_tap_shift; kwargs...)
end

""
function _build_opf_tap_shift(pm::AbstractPowerModel)
    variable_voltage(pm)
    variable_generation(pm)
