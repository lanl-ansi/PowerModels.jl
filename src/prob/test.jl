...
"multi-network opf with mi storage variables"
function _run_mn_strg_mi_opf(file, model_constructor, optimizer; kwargs...)
    return run_model(file, model_constructor, optimizer, _post_mn_strg_mi_opf; multinetwork=true, kwargs...)
end

""
function _post_mn_strg_mi_opf(pm::GenericPowerModel)
    for (n, network) in nws(pm)
        variable_voltage(pm, nw=n)
        variable_generation(pm, nw=n)
