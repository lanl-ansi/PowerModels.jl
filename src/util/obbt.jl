...
function solve_obbt_opf!(data::Dict{String,<:Any}, optimizer;
    ...
    @_info("maximum OBBT iterations set to default value of $max_iter")
    @_info("maximum time limit for OBBT set to default value of $time_limit seconds")

    model_relaxation = instantiate_model(data, model_type, PowerModels.build_opf)
    (_IM.ismultinetwork(model_relaxation, pm_it_sym)) && (@_error("OBBT is not supported for multi-networks"))
    ...
    (termination != :avg && termination != :max) && (@_error("OBBT termination criteria can only be :max or :avg"))
    ...
    if upper_bound < current_relaxation_objective
        @_error("the upper bound provided to OBBT is not a valid ACOPF upper bound")
    end
    if !(result_relaxation["termination_status"] in status_pass)
        @_warn("initial relaxation solve status is $(result_relaxation["termination_status"])")
        if result_relaxation["termination_status"] == :SubOptimal
            @_warn("continuing with the bound-tightening algorithm")
        end
    end
    current_rel_gap = Inf
    if !isinf(upper_bound)
        current_rel_gap = (upper_bound - current_relaxation_objective)/upper_bound
        @_info("Initial relaxation gap = $current_rel_gap")
    end
    ...
    while check_termination
        ...
        for bus in buses
            ...
            if (result_bt["termination_status"] == JuMP.LOCALLY_SOLVED || result_bt["termination_status"] == JuMP.OPTIMAL)
                ...
                @_warn("BT minimization problem for vm[$bus] errored - change tolerances.")
                continue
            end
            ...
            if (result_bt["termination_status"] == JuMP.LOCALLY_SOLVED || result_bt["termination_status"] == JuMP.OPTIMAL)
                ...
                @_warn("BT maximization problem for vm[$bus] errored - change tolerances.")
                continue
            end
            ...
            # sanity checks
            (lb > ub) && (@_warn("bt lb > ub - adjust tolerances in optimizer to avoid issue"); continue)
        ...
        for bp in buspairs
            ...
            if (result_bt["termination_status"] == JuMP.LOCALLY_SOLVED || result_bt["termination_status"] == JuMP.OPTIMAL)
                ...
                @_warn("BT minimization problem for td[$bp] errored - change tolerances")
                continue
            end
            ...
            if (result_bt["termination_status"] == JuMP.LOCALLY_SOLVED || result_bt["termination_status"] == JuMP.OPTIMAL)
                ...
                @_warn("BT maximization problem for td[$bp] errored - change tolerances.")
                continue
            end
            end_time = time() - start_time
        ...
        if result_relaxation["termination_status"] in status_pass
            ...
            @_warn("relaxation solve failed in iteration $(current_iteration+1)")
            @_warn("using the previous iteration's gap to check relative gap stopping criteria")
        end

        @_info("iteration $(current_iteration+1), vm range: $vm_range_final, td range: $td_range_final, relaxation obj: $final_relaxation_objective")
        ...
        (current_iteration >= max_iter) && (@_info("maximum iteration limit reached"); break)
        (time_elapsed > time_limit) && (@_info("maximum time limit reached"); break)
        if (!isinf(rel_gap_tol)) && (current_rel_gap < rel_gap_tol)
            @_info("relative optimality gap < $rel_gap_tol")
            break
        end
...
function _check_variables(pm::AbstractPowerModel)
    try
        ...
        (isa(error, KeyError)) && (@_error("OBBT is not supported for models without explicit voltage magnitude variables"))
    end

    try
        td = var(pm, :td)
    catch err
        (isa(error, KeyError)) && (@_error("OBBT is not supported for models without explicit voltage angle difference variables"))
    end
end
...
function _check_obbt_options(ub::Float64, rel_gap::Float64, ub_constraint::Bool)
    if ub_constraint && isinf(ub)
        @_error("the option upper_bound_constraint cannot be set to true without specifying an upper bound")
    end

    if !isinf(rel_gap) && isinf(ub)
        @_error("rel_gap_tol is specified without providing an upper bound")
    end
end
        ...
        for (i,gen) in nw_ref[:gen]
            if haskey(gen, "cost")
                if model == nothing
                    ...
                    if gen["model"] != model
                        @_error("cost models are inconsistent, the typical model is $(model) however model $(gen["model"]) is given on generator $(i)")
                    end
                end
            else
                @_error("no cost given for generator $(i)")
            end
        end
            ...
            if haskey(dcline, "model")
                if model == nothing
                    ...
                    if dcline["model"] != model
                        @_error("cost models are inconsistent, the typical model is $(model) however model $(dcline["model"]) is given on dcline $(i)")
                    end
                end
            else
                @_error("no cost given for dcline $(i)")
            end
        end
    end

    if model != 2
        @_error("Only cost models of type 2 is supported at this time, given cost model type $(model)")
    end

    cost_index = PowerModels.calc_max_cost_index(pm.data)
    if cost_index > 3
        @_error("Only quadratic generator cost models are supported at this time, given cost model of order $(cost_index-1)")
    end
