...
function export_pti(io::IO, data::Dict{String,Any})
    ...
    for item in incompatible_items 
        if haskey(data, item) && length(data[item]) > 0
            ...
            @warn "Skipping export of the $(components) $(item) items because it is not suported in the PSSE 33 .raw file"
        end
    end
    ...
    for item in not_exported_items 
        if haskey(data, item)
            components = length(data[item])
            if components > 0
                @warn "Skipping export of the $(components) $(item) data because is not yet implemented"
            end
        end
    end
