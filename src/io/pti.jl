...
function _get_line_elements(line::AbstractString)
    if count(i->(i=="'"), line) % 2 == 1
        throw(@error("There are an uneven number of single-quotes in \"{line}\", the line cannot be parsed."))
    end
    ...
    for (line_number, line) in enumerate(data_lines)
        ...
        elseif line_number > 3 && length(elements) != 0 && first_element == "0"
            ...
            if length(elements) > 1
                @warn("At line $line_number, new section started with '0', but additional non-comment data is present. Pattern '^\\s*0\\s*[/]*.*' is reserved for section start/end.")
...
function parse_pti(io::IO)::Dict
    ...
    try
        ...
        @info("unable to recover case name from io file name in parse_pti")
    end
...
function export_pti(io::IO, data::Dict{String,Any})
    ...
    for item in incompatible_items 
        if haskey(data, item) && length(data[item]) > 0
            ...
            @warn(string("Skipping export of the $(components) $(item) items because it is not suported in the PSSE 33 .raw file")) 
        end
    end
    ...
    for item in not_exported_items 
        if haskey(data, item)
            components = length(data[item])
            if components > 0
                @warn(string("Skipping export of the $(components) $(item) data because is not yet implemented")) 
            end
        end
    end
