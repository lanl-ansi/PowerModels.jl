...
function _parse_matpower_string(data_string::String)
    ...
    if haskey(matlab_data, "mpc.bus_name")
        ...
        if length(case["bus_name"]) != length(case["bus"])
            @_error("incorrect Matpower file, the number of bus names ($(length(case["bus_name"]))) is inconsistent with the number of buses ($(length(case["bus"]))).\n")
        end
    end
    ...
    if haskey(matlab_data, "mpc.gencost")
        ...
        if length(case["gencost"]) != length(case["gen"]) && length(case["gencost"]) != 2*length(case["gen"])
            @_error("incorrect Matpower file, the number of generator cost functions ($(length(case["gencost"]))) is inconsistent with the number of generators ($(length(case["gen"]))).\n")
        end
    end
    ...
    if haskey(matlab_data, "mpc.dclinecost")
        ...
        if length(case["dclinecost"]) != length(case["dcline"])
            @_error("incorrect Matpower file, the number of dcline cost functions ($(length(case["dclinecost"]))) is inconsistent with the number of dclines ($(length(case["dcline"]))).\n")
        end
    end
    ...
    for k in keys(matlab_data)
        if !in(k, _mp_data_names) && startswith(k, "mpc.")
            ...
            if isa(value, Array)
                ...
                case[case_name] = tbl
                @_info("extending matpower format with data: $(case_name) $(length(tbl))x$(length(tbl[1])-1)")
            else
                case[case_name] = value
                @_info("extending matpower format with constant data: $(case_name)")
            end
        end
    end
...
function _mp_cost_data(cost_row)
    ...
    if cost_data["model"] == 1:
        if length(cost_values)%2 != 0
            @_error("incorrect matpower file, odd number of pwl cost function values")
        end
...
function _add_dcline_costs!(data::Dict{String,Any})
    if length(data["gencost"]) > 0 && length(data["dclinecost"]) <= 0 && length(data["dcline"]) > 0
        @_warn("added zero cost function data for dclines")
...
function _merge_cost_data!(data::Dict{String,Any})
    if haskey(data, "gencost")
        ...
        if length(gen) != length(gencost)
            if length(gencost) > length(gen)
                @_warn("The last $(length(gencost) - length(gen)) generator cost records will be ignored due to too few generator records.")
                gencost = gencost[1:length(gen)]
            else
                @_warn("The number of generators ($(length(gen))) does not match the number of generator cost records ($(length(gencost))).")
            end
        end
    ...
    if haskey(data, "dclinecost")
        ...
        if length(dcline) != length(dclinecost)
            @_warn("The number of dclines ($(length(dcline))) does not match the number of dcline cost records ($(length(dclinecost))).")
        end
            ...
            for mp_name in mp_matrix_names
                if startswith(k, "$(mp_name)_")
                    ...
                    if length(mp_matrix) != length(v)
                        @_error("failed to extend the matpower matrix \"$(mp_name)\" with the matrix \"$(k)\" because they do not have the same number of rows, $(length(mp_matrix)) and $(length(v)) respectively.")
                    end

                    @_info("extending matpower format by appending matrix \"$(k)\" in to \"$(mp_name)\"")
                    ...
                    for (i, row) in enumerate(mp_matrix)
                        ...
                        for key in keys(merge_row)
                            if haskey(row, key)
                                @_error("failed to extend the matpower matrix \"$(mp_name)\" with the matrix \"$(k)\" because they both share \"$(key)\" as a column name.")
                            end
...
function _check_keys(data, keys)
    for key in keys
        if haskey(data, key)
            @_error("attempting to overwrite value of $(key) in PowerModels data,\n$(data)")
        end
    end
end
...
function export_matpower(io::IO, data::Dict{String,Any})
    if _IM.ismultinetwork(data)
        @_error("export_matpower does not yet support multinetwork data")
    end
    ...
    for (idx,gen) in sort(collect(generators), by=(x) -> x.first)
        if idx != gen["index"]
            @_warn("The index of the generator does not match the matpower assigned index. Any data that uses generator indexes for reference is corrupted.");
        end
        println(io,
            "\t", gen["gen_bus"],
            "\t", _get_default(gen, "pg"),
    ...
    if length(storage) > 0
        ...
        for (idx,strg) in sort(collect(storage), by=(x) -> x.first)
            if idx != strg["index"]
                @_warn("The index of the storage does not match the matpower assigned index. Any data that uses storage indexes for reference is corrupted.");
            end
    ...
    for (idx,branch) in sort(collect(branches), by=(x) -> x.first)
        if idx != branch["index"]
            @_warn("The index of the branch does not match the matpower assigned index. Any data that uses branch indexes for reference is corrupted.");
        end
        println(io,
    ...
    if haskey(data, "ne_branch")
        ...
        for (idx,branch) in sort(collect(ne_branches), by=(x) -> x.first)
            if idx != branch["index"]
                @_warn("The index of the ne_branch does not match the matpower assigned index. Any data that uses branch indexes for reference is corrupted.");
            end
            println(io,
                "\t", branch["f_bus"],
                "\t", branch["t_bus"],
...
function _export_extra_data(io::IO, data::Dict{String,<:Any}, component, excluded_fields=Set(["index", "source_id"]); postfix="")
    ...
    for (key, value) in data[component]
        if !isa(value, Dict)
            @_warn("skipping export $(component), does not appear to be a component Dict")
            return
        end
    end
    ...
    for idx in key_order
        ...
        if haskey(c, "index") && i != c["index"]
            @_warn("The index of a component does not match the implicit matpower index. Any data that uses component indexes for reference is corrupted.")
        end
        ...
        for field in included_fields
            ...
            if haskey(c, field)
                ...
                elseif typeof(value) <: Real
                    ...
                    @_warn("unable to export $(typeof(value)) value for '$(field)' in component list $(component), not a string or numeric value")
                    print(io, "NaN")
                end
            else
                print(io, "NaN")
    ...
    if haskey(a_comp, "cost")
        ...
        for (i,comp) in components
            if length(comp["cost"]) != ncost || comp["model"] != model
                @_warn("heterogeneous cost functions will be ommited in Matpower data")
                return
            end
        end
