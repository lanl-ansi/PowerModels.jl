...
function export_matpower(io::IO, data::Dict{String,Any})
    ...
    for (idx,bus) in sort(buses)
        println(io, "\t", bus["index"], 
                    "\t", bus["bus_type"], 
                    "\t", get_default(pd, bus["index"]), # != NaN ? pd[bus["index"]] : 0 , 
                    "\t", get_default(qd, bus["index"]), # qd[bus["index"]] != NaN ? qd[bus["index"]] : 0, 
                    "\t", get_default(gs, bus["index"]), # gs[bus["index"]] != NaN ? gs[bus["index"]] : 0, 
                    "\t", get_default(bs, bus["index"]), #bs[bus["index"]] != NaN ? bs[bus["index"]] : 0, 
                    "\t", get_default(bus, "area"), #bus["area"] != NaN ? bus["area"] : 0, 
                    "\t", get_default(bus, "vm"), #bus["vm"] != NaN ? bus["vm"] : 0, 
                    "\t", get_default(bus, "va"), #bus["va"] != NaN ? bus["va"] : 0, 
                    "\t", get_default(bus, "base_kv"), #bus["base_kv"] != NaN ? bus["base_kv"] : 0, 
                    "\t", get_default(bus, "zone"), #bus["zone"] != NaN ? bus["zone"] : 0, 
                    "\t", get_default(bus, "vmax"), #bus["vmax"] != NaN ? bus["vmax"] : 0, 
                    "\t", get_default(bus, "vmin"), #bus["vmin"] != NaN ? bus["vmin"] : 0,
                    "\t", get_default(bus, "lam_p", ""), #(haskey(bus,"lam_p") ? bus["lam_p"] : ""),   
                    "\t", get_default(bus, "lam_q", ""), #(haskey(bus,"lam_q") ? bus["lam_q"] : ""),   
                    "\t", get_default(bus, "mu_vmax", ""), #(haskey(bus,"mu_vmax") ? bus["mu_vmax"] : ""),   
                    "\t", get_default(bus, "mu_vmin",""), #(haskey(bus,"mu_vmin") ? bus["mu_vmin"] : ""),   
                    )
    end  
    println(io, "];")
    println(io)  
    ...
    for (idx,gen) in sort(generators)
        ...
        println(io, "\t", gen["gen_bus"], 
                    "\t", get_default(gen, "pg"), #gen["pg"] != NaN ? gen["pg"] : 0,
                    "\t", get_default(gen, "qg"), #gen["qg"] != NaN ? gen["qg"] : 0,
                    "\t", get_default(gen, "qmax"), #gen["qmax"] != NaN ? gen["qmax"] : 0,
                    "\t", get_default(gen, "qmin"), #gen["qmin"] != NaN ? gen["qmin"] : 0,
                    "\t", get_default(gen, "vg"), #gen["vg"] != NaN ? gen["vg"] : 0,
                    "\t", get_default(gen, "mbase"), #gen["mbase"] != NaN ? gen["mbase"] : 0,
                    "\t", get_default(gen, "gen_status"), #gen["gen_status"] != NaN ? gen["gen_status"] : 0,
                    "\t", get_default(gen, "pmax"), #gen["pmax"] != NaN ? gen["pmax"] : 0,
                    "\t", get_default(gen, "pmin"), #gen["pmin"] != NaN ? gen["pmin"] : 0,
                    "\t", get_default(gen, "pc1", ""), #(haskey(gen, "pc1") ? gen["pc1"] : ""),
                    "\t", get_default(gen, "pc2", ""), #(haskey(gen, "pc2") ? gen["pc2"] : ""),
                    "\t", get_default(gen, "qc1min", ""), #(haskey(gen, "qc1min") ? gen["qc1min"] : ""),
                    "\t", get_default(gen, "qc1max", ""), #(haskey(gen, "qc1max") ? gen["qc1max"] : ""),
                    "\t", get_default(gen, "qc2min", ""), #(haskey(gen, "qc2min") ? gen["qc2min"] : ""),
                    "\t", get_default(gen, "qc2max", ""), #(haskey(gen, "qc2max") ? gen["qc2max"] : ""),
                    "\t", get_default(gen, "ramp_agc", ""), #(haskey(gen, "ramp_agc") ? gen["ramp_agc"] : ""),
                    "\t", (haskey(gen, "ramp_10") ? gen["ramp_10"] : haskey(gen, "ramp_30") ? 0 : ""),
                    "\t", get_default(gen, "ramp_30", ""), #(haskey(gen, "ramp_30") ? gen["ramp_30"] : ""),
                    "\t", get_default(gen, "ramp_q", ""), #(haskey(gen, "ramp_q") ? gen["ramp_q"] : ""), 
                    "\t", get_default(gen, "apf", ""), #(haskey(gen, "apf") ? gen["apf"] : ""),
                    "\t", get_default(gen, "mu_pmax", ""), #(haskey(gen, "mu_pmax") ? gen["mu_pmax"] : ""),
                    "\t", get_default(gen, "mu_pmin", ""), #(haskey(gen, "mu_pmin") ? gen["mu_pmin"] : ""),
                    "\t", get_default(gen, "mu_qmax", ""), #(haskey(gen, "mu_qmax") ? gen["mu_qmax"] : ""),
                    "\t", get_default(gen, "mu_qmin", ""), #(haskey(gen, "mu_qmin") ? gen["mu_qmin"] : ""),
                    )      
        i = i+1
    end
    println(io,"];")
    ...
    for (idx,branch) in sort(branches)
        ...
        println(io,
            "\t", get_default(branch, "f_bus"), #branch["f_bus"] != NaN ? branch["f_bus"] : 0, 
            "\t", get_default(branch, "t_bus"), #branch["t_bus"] != NaN ? branch["t_bus"] : 0, 
            "\t", get_default(branch, "br_r"), #branch["br_r"] != NaN ? branch["br_r"] : 0, 
            "\t", get_default(branch, "br_x"), #branch["br_x"] != NaN ? branch["br_x"] : 0, 
            "\t", (branch["b_to"] + branch["b_fr"]) != NaN ? (branch["b_to"] + branch["b_fr"]) : 0, 
            "\t", get_default(branch, "rate_a"), #(haskey(branch, "rate_a") ? branch["rate_a"] : 0),
            "\t", get_default(branch, "rate_b"), #(haskey(branch, "rate_b") ? branch["rate_b"] : 0),
            "\t", get_default(branch, "rate_c"), #(haskey(branch, "rate_c") ? branch["rate_c"] : 0),
            "\t", (branch["transformer"] ? branch["tap"] : 0),
            "\t", (branch["transformer"] ? branch["shift"] : 0),
            "\t", get_default(branch, "br_status"), #branch["br_status"] != NaN ? branch["br_status"] : 0,
            "\t", get_default(branch, "angmin"), #branch["angmin"] != NaN ? branch["angmin"] : 0,
            "\t", get_default(branch, "angmax"), #branch["angmax"] != NaN ? branch["angmax"] : 0,
            "\t", get_default(branch, "pf", ""), #(haskey(branch, "pf") ? branch["pf"] : ""),
            "\t", get_default(branch, "qf", ""), #(haskey(branch, "qf") ? branch["qf"] : ""),
            "\t", get_default(branch, "pt", ""), #(haskey(branch, "pt") ? branch["pt"] : ""),
            "\t", get_default(branch, "qt", ""), #(haskey(branch, "qt") ? branch["qt"] : ""),
            "\t", get_default(branch, "mu_sf", ""), #(haskey(branch, "mu_sf") ? branch["mu_sf"] : ""),
            "\t", get_default(branch, "mu_st", ""), #(haskey(branch, "mu_st") ? branch["mu_st"] : ""),
            "\t", get_default(branch, "mu_angmin", ""), #(haskey(branch, "mu_angmin") ? branch["mu_angmin"] : ""),
            "\t", get_default(branch, "mu_angmax", ""), #(haskey(branch, "mu_angmax") ? branch["mu_angmax"] : ""),
        )
        i = i+1
    end
    println(io, "];")
    ...
    for (idx, dcline) in sort(dclines)
        println(io,
            "\t", get_default(dcline, "f_bus"), # dcline["f_bus"] != NaN ? dcline["f_bus"] : 0,
            "\t", get_default(dcline, "t_bus"), # dcline["t_bus"] != NaN ? dcline["t_bus"] : 0,
            "\t", get_default(dcline, "br_status"), # dcline["br_status"] != NaN ? dcline["br_status"] : 0,
            "\t", get_default(dcline, "pf"), # dcline["pf"] != NaN ? dcline["pf"] : 0,
            "\t", -get_default(dcline, "pt"), #(-dcline["pt"]) != NaN ? -dcline["pt"] : 0, # opposite convention
            "\t", -get_default(dcline, "qf"), #(-dcline["qf"]) != NaN ? -dcline["qf"] : 0, # opposite convention
            "\t", -get_default(dcline, "qt"), # (-dcline["qt"]) != NaN ? -dcline["qt"] : 0, # opposite convention
            "\t", get_default(dcline, "vf"), #dcline["vf"] != NaN ? dcline["vf"] : 0,
            "\t", get_default(dcline, "vt"), # dcline["vt"] != NaN ? dcline["vt"] : 0,
            ...
            "\t", get_default(dcline, "qminf"), # dcline["qminf"] != NaN ? dcline["qminf"] : 0,
            "\t", get_default(dcline, "qmaxf"), #dcline["qmaxf"] != NaN ? dcline["qmaxf"] : 0,
            "\t", get_default(dcline, "qmint"), #dcline["qmint"] != NaN ? dcline["qmint"] : 0,
            "\t", get_default(dcline, "qmaxt"), #dcline["qmaxt"] != NaN ? dcline["qmaxt"] : 0,
            "\t", get_default(dcline, "loss0"), #dcline["loss0"] != NaN ? dcline["loss0"] : 0,
            "\t", get_default(dcline, "loss1"), #dcline["loss1"] != NaN ? dcline["loss1"] : 0,
            "\t", get_default(dcline, "mu_pmin", ""), #(haskey(dcline, "mu_pmin") ? (dcline["mu_pmin"]) : ""),
            "\t", get_default(dcline, "mu_pmax", ""), #(haskey(dcline, "mu_pmax") ? (dcline["mu_pmax"]) : ""),
            "\t", get_default(dcline, "mu_qminf", ""), #(haskey(dcline, "mu_qminf") ? (dcline["mu_qminf"]) : ""),
            "\t", get_default(dcline, "mu_qmaxf", ""), #(haskey(dcline, "mu_qmaxf") ? (dcline["mu_qmaxf"]) : ""),
            "\t", get_default(dcline, "mu_qmint", ""), #(haskey(dcline, "mu_qmint") ? (dcline["mu_qmint"]) : ""),
            "\t", get_default(dcline, "mu_qmaxt", ""), #(haskey(dcline, "mu_qmaxt") ? (dcline["mu_qmaxt"]) : ""),
        )
    end     
    println(io, "];")
    println(io)
    ...
    if haskey(data, "ne_branch")
        ...
        for (idx,branch) in sort(ne_branches)
            ...
            println(io, 
                ...
                "\t", get_default(branch, "angmin"), # branch["angmin"] != NaN ? branch["angmin"] : 0,
                "\t", get_default(branch, "angmax"), #branch["angmax"] != NaN ? branch["angmax"] : 0,
                "\t", get_default(branch, "construction_cost"), #branch["construction_cost"] != NaN ? branch["construction_cost"] : 0,
            )
            i = i+1
        end
        println(io, "];");
