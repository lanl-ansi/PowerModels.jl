...
function cut_complex_product_and_angle_difference(m, wf, wt, wr, wi, angmin, angmax)
    ...
    vfub = sqrt(JuMP.upperbound(wf))
    vflb = sqrt(JuMP.lowerbound(wf))
    vtub = sqrt(JuMP.upperbound(wt))
    vtlb = sqrt(JuMP.lowerbound(wt))
    tdub = angmax
    tdlb = angmin
...
function relaxation_sin(m, x, y)
    ub = JuMP.upperbound(x)
    lb = JuMP.lowerbound(x)
