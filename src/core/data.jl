...
function check_dc_line_limits(data)
    ...
    for (i, dcline) in data["dcline"]
      ...
      if dcline["loss0"] >= dcline["pmaxf"]*(1-dcline["loss1"] )+ dcline["pmaxt"]
        ...
        if dcline["loss1"] < 0.0
            new_rate = 0.0
            warn("this code only supports positive loss1 values, changing the value on dcline $(dcline["index"]) from $(mva_base*dcline["loss1"]) to $(mva_base*new_rate)")
            branch["loss1"] = new_rate
        end

        if dcline["loss1"] >= 1.0
            new_rate = 0.0
            warn("this code only supports loss1 values < 1, changing the value on dcline $(dcline["index"]) from $(mva_base*dcline["loss1"]) to $(mva_base*new_rate)")
            branch["loss1"] = new_rate
        end

        if dcline["pmint"] <0.0 && dcline["loss1"] > 0.0
            #new_rate = 0.0
            warn("the dc line model is not meant to be used bi-directionally when loss0 > 0, be careful interpreting the results as the dc line losses can now be negative")
            #branch["loss0"] = new_rate
        end
