...
function correct_voltage_angle_differences!(data::Dict{String,<:Any}, default_pad = 1.0472)
    ...
    for c in 1:get(pm_data, "conductors", 1)
        ...
        for (i, branch) in pm_data["branch"]
            ...
            if angmin <= -pi/2
                @warn("this code only supports angmin values in -90 deg. to 90 deg., tightening the value on branch $i$(cnd_str) from $(rad2deg(angmin)) to -$(default_pad_deg) deg.")
            ...
            if angmax >= pi/2
                @warn("this code only supports angmax values in -90 deg. to 90 deg., tightening the value on branch $i$(cnd_str) from $(rad2deg(angmax)) to $(default_pad_deg) deg.")
                if haskey(pm_data, "conductors")
            ...
            if angmin == 0.0 && angmax == 0.0
                @warn("angmin and angmax values are 0, widening these values on branch $i$(cnd_str) to +/- $(default_pad_deg) deg.")
                if haskey(pm_data, "conductors")
                    branch["angmin"][c] = -default_pad
                    branch["angmax"][c] =  default_pad
                else
    ...
    for branch in branches
        ...
        for c in 1:get(pm_data, "conductors", 1)
            ...
            if branch["rate_a"][c] <= 0.0
                ...
                @warn("this code only supports positive rate_a values, changing the value on branch $(branch["index"])$(cnd_str) to $(round(mva_base*new_rate, digits=4))")
...
function _check_storage_parameters(data::Dict{String,<:Any})
    for (i, strg) in data["storage"]
        ...
        for c in 1:get(data, "conductors", 1)
            if strg["r"][c] < 0.0
                @error("storage unit $(strg["index"]) has a non-positive resistance $(strg["r"][c])")
            end
            if strg["x"][c] < 0.0
                @error("storage unit $(strg["index"]) has a non-positive reactance $(strg["x"][c])")
            end
            if haskey(strg, "thermal_rating") && strg["thermal_rating"][c] < 0.0
                @error("storage unit $(strg["index"]) has a non-positive thermal rating $(strg["thermal_rating"][c])")
            end
            if haskey(strg, "current_rating") && strg["current_rating"][c] < 0.0
                @error("storage unit $(strg["index"]) has a non-positive current rating $(strg["thermal_rating"][c])")
            end
        end
