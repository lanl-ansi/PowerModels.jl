...
function calc_pwl_points(ncost::Int, cost::Vector{<:Real}, pmin::Real, pmax::Real; tolerance=1e-2)
    ...
    if isinf(pmin) || isinf(pmax)
        @log_error("a bounded operating range is required for modeling pwl costs.  Given active power range in $(pmin) - $(pmax)")
    end
    ...
    for (n, nw_ref) in nws(pm)
        ...
        for (i,gen) in ref(pm, n, :gen)
            ...
            elseif gen["model"] == 2
                ...
                @log_error("Only cost models of types 1 and 2 are supported at this time, given cost model type of $(model) on generator $(i)")
            end
        end
    ...
    for (n, nw_ref) in nws(pm)
        ...
        for (i,dcline) in ref(pm, n, :dcline)
            ...
            elseif dcline["model"] == 2
                ...
                @log_error("only cost models of types 1 and 2 are supported at this time, given cost model type of $(model) on dcline $(i)")
            end
        end
...
function _polynomial_cost_expression(pm::AbstractConicModels, x_list::Array{JuMP.VariableRef}, cost_terms; nw=0, id=1, var_name="x")
    ...
    elseif length(cost_terms) == 3
        ...
        @log_error("the network cost data features a polynomial cost function that is not compatible with conic mathematical programs.")
    end
end
