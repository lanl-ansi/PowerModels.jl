...
function variable_phase_angle{T}(pm::GenericPowerModel{T}; bounded = true)
    @variable(pm.model, t[i in keys(pm.set.buses)], start = getstart(pm.set.buses, i, "t_start"))
    return t
end

function variable_voltage_magnitude{T}(pm::GenericPowerModel{T}; bounded = true)
    if bounded
        @variable(pm.model, pm.set.buses[i]["vmin"] <= v[i in keys(pm.set.buses)] <= pm.set.buses[i]["vmax"], start = getstart(pm.set.buses, i, "v_start", 1.0))
    else
        @variable(pm.model, v[i in keys(pm.set.buses)] >= 0, start = getstart(pm.set.buses, i, "v_start", 1.0))
    end
    return v
end
...
function variable_voltage_magnitude_sqr{T}(pm::GenericPowerModel{T}; bounded = true)
    if bounded
        @variable(pm.model, pm.set.buses[i]["vmin"]^2 <= w[i in keys(pm.set.buses)] <= pm.set.buses[i]["vmax"]^2, start = getstart(pm.set.buses, i, "w_start", 1.001))
    else
        @variable(pm.model, w[i in keys(pm.set.buses)] >= 0, start = getstart(pm.set.buses, i, "w_start", 1.001))
    end
    return w
end
...
function variable_voltage_magnitude_sqr_from_on_off{T}(pm::GenericPowerModel{T})
    ...
    @variable(pm.model, 0 <= w_from[i in keys(pm.set.branches)] <= buses[branches[i]["f_bus"]]["vmax"]^2, start = getstart(pm.set.buses, i, "w_from_start", 1.001))
...
function variable_voltage_magnitude_sqr_to_on_off{T}(pm::GenericPowerModel{T})
    ...
    @variable(pm.model, 0 <= w_to[i in keys(pm.set.branches)] <= buses[branches[i]["t_bus"]]["vmax"]^2, start = getstart(pm.set.buses, i, "w_to", 1.001))
...
function variable_active_generation{T}(pm::GenericPowerModel{T}; bounded = true)
    if bounded
        @variable(pm.model, pm.set.gens[i]["pmin"] <= pg[i in keys(pm.set.gens)] <= pm.set.gens[i]["pmax"], start = getstart(pm.set.gens, i, "pg_start"))
    else
        @variable(pm.model, pg[i in keys(pm.set.gens)], start = getstart(pm.set.gens, i, "pg_start"))
    end
    return pg
end
...
function variable_reactive_generation{T}(pm::GenericPowerModel{T}; bounded = true)
    if bounded
        @variable(pm.model, pm.set.gens[i]["qmin"] <= qg[i in keys(pm.set.gens)] <= pm.set.gens[i]["qmax"], start = getstart(pm.set.gens, i, "qg_start"))
    else
        @variable(pm.model, qg[i in keys(pm.set.gens)], start = getstart(pm.set.gens, i, "qg_start"))
    end
    return qg
end
...
function variable_complex_voltage_product{T}(pm::GenericPowerModel{T}; bounded = true)
    if bounded
        ...
        @variable(pm.model, wr_min[bp] <= wr[bp in keys(pm.set.buspairs)] <= wr_max[bp], start = getstart(pm.set.buspairs, bp, "wr_start", 1.0))
        @variable(pm.model, wi_min[bp] <= wi[bp in keys(pm.set.buspairs)] <= wi_max[bp], start = getstart(pm.set.buspairs, bp, "wi_start"))
    else
        @variable(pm.model, wr[bp in keys(pm.set.buspairs)], start = getstart(pm.set.buspairs, bp, "wr_start", 1.0))
        @variable(pm.model, wi[bp in keys(pm.set.buspairs)], start = getstart(pm.set.buspairs, bp, "wi_start"))
    end
    return wr, wi
end
...
function variable_complex_voltage_product_on_off{T}(pm::GenericPowerModel{T})
    ...
    @variable(pm.model, min(0, wr_min[bi_bp[b]]) <= wr[b in keys(pm.set.branches)] <= max(0, wr_max[bi_bp[b]]), start = getstart(pm.set.buspairs, bi_bp[b], "wr_start", 1.0))
    @variable(pm.model, min(0, wi_min[bi_bp[b]]) <= wi[b in keys(pm.set.branches)] <= max(0, wi_max[bi_bp[b]]), start = getstart(pm.set.buspairs, bi_bp[b], "wi_start"))
...
function variable_complex_voltage_product_matrix{T}(pm::GenericPowerModel{T})
    ...
    w_index = 1:length(keys(pm.set.buses))
    lookup_w_index = Dict([(bi, i) for (i,bi) in enumerate(keys(pm.set.buses))])

    @variable(pm.model, WR[1:length(keys(pm.set.buses)), 1:length(keys(pm.set.buses))], Symmetric)
    @variable(pm.model, WI[1:length(keys(pm.set.buses)), 1:length(keys(pm.set.buses))])
...
function variable_phase_angle_difference{T}(pm::GenericPowerModel{T})
    @variable(pm.model, pm.set.buspairs[bp]["angmin"] <= td[bp in keys(pm.set.buspairs)] <= pm.set.buspairs[bp]["angmax"], start = getstart(pm.set.buspairs, bp, "td_start"))
    return td
end

# Creates the voltage magnitude product variables
function variable_voltage_magnitude_product{T}(pm::GenericPowerModel{T})
    vv_min = Dict([(bp, buspair["v_from_min"]*buspair["v_to_min"]) for (bp, buspair) in pm.set.buspairs])
    vv_max = Dict([(bp, buspair["v_from_max"]*buspair["v_to_max"]) for (bp, buspair) in pm.set.buspairs])

    @variable(pm.model,  vv_min[bp] <= vv[bp in keys(pm.set.buspairs)] <=  vv_max[bp], start = getstart(pm.set.buspairs, bp, "vv_start", 1.0))
    return vv
end

function variable_cosine{T}(pm::GenericPowerModel{T})
    cos_min = Dict([(bp, -Inf) for bp in keys(pm.set.buspairs)])
    cos_max = Dict([(bp,  Inf) for bp in keys(pm.set.buspairs)])

    for (bp, buspair) in pm.set.buspairs
        if buspair["angmin"] >= 0
    ...
    @variable(pm.model, cos_min[bp] <= cs[bp in keys(pm.set.buspairs)] <= cos_max[bp], start = getstart(pm.set.buspairs, bp, "cs_start", 1.0))
    return cs
end

function variable_sine{T}(pm::GenericPowerModel{T})
    @variable(pm.model, sin(pm.set.buspairs[bp]["angmin"]) <= si[bp in keys(pm.set.buspairs)] <= sin(pm.set.buspairs[bp]["angmax"]), start = getstart(pm.set.buspairs, bp, "si_start"))
    return si
end

function variable_current_magnitude_sqr{T}(pm::GenericPowerModel{T})
    buspairs = pm.set.buspairs
    cm_min = Dict([(bp, 0) for bp in keys(pm.set.buspairs)])
    cm_max = Dict([(bp, (buspair["rate_a"]*buspair["tap"]/buspair["v_from_min"])^2) for (bp, buspair) in pm.set.buspairs])

    @variable(pm.model, cm_min[bp] <= cm[bp in keys(pm.set.buspairs)] <=  cm_max[bp], start = getstart(pm.set.buspairs, bp, "cm_start"))
    return cm
end

function variable_line_indicator{T}(pm::GenericPowerModel{T})
    @variable(pm.model, 0 <= line_z[l in keys(pm.set.branches)] <= 1, Int, start = getstart(pm.set.branches, l, "line_z_start", 1.0))
    return line_z
end
