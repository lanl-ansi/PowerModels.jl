...
function solve_model(data::Dict{String,<:Any}, model_type::Type, optimizer, build_method;
    ...
    if multinetwork != _IM.ismultinetwork(data)
        ...
        @_error("attempted to build a $(model_requirement) model with $(data_type) data")
    end
...
function ref_add_core!(ref::Dict{Symbol,Any})
    for (nw, nw_ref) in ref[:it][pm_it_sym][:nw]
        ...
        if length(ref_buses) > 1
            @_warn("multiple reference buses found, $(keys(ref_buses)), this can cause infeasibility if they are in the same connected component")
        end
