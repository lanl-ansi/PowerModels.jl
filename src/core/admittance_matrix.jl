...
function calc_admittance_matrix(data::Dict{String,<:Any})
    if length(data["dcline"]) > 0
        @_error("calc_admittance_matrix does not support data with dclines")
    end
    if length(data["switch"]) > 0
        @_error("calc_admittance_matrix does not support data with switches")
    end
    ...
    for bus in buses
        ...
        if bus_type == 1
            push!(idx_to_J_idx, running_idx)
            running_idx += 2 # bus of type 1 needs two rows and cols 
        elseif bus_type == 2
            push!(idx_to_J_idx, running_idx)
            running_idx += 1 # bus of type 2 needs one row and col 
        elseif bus_type == 3
            push!(idx_to_J_idx, 0)
            # type 3 bus needs no rows and cols
        else 
...
function calc_susceptance_matrix(data::Dict{String,<:Any})
    if length(data["dcline"]) > 0
        @_error("calc_susceptance_matrix does not support data with dclines")
    end
    if length(data["switch"]) > 0
        @_error("calc_susceptance_matrix does not support data with switches")
    end
...
function calc_susceptance_matrix_inv(data::Dict{String,<:Any})
    ...
    if !(ref_idx > 0 && ref_idx <= num_buses)
        @_error("invalid ref_idx in calc_susceptance_matrix_inv")
    end
    S[ref_idx, :] .= 0.0
    S[:, ref_idx] .= 0.0
    ...
    if !LinearAlgebra.issuccess(F)
        @_error("Failed factorization in calc_susceptance_matrix_inv")
    end
...
function calc_admittance_matrix_inv(am::AdmittanceMatrix{T}, ref_idx::Int) where {T}
    ...
    if !(ref_idx > 0 && ref_idx <= num_buses)
        @_error("invalid ref_idx in calc_admittance_matrix_inv")
    end
...
function calc_bus_injection(data::Dict{String,<:Any})
    if length(data["dcline"]) > 0
        @_error("calc_bus_injection does not support data with dclines")
    end
    if length(data["switch"]) > 0
        @_error("calc_bus_injection does not support data with switches")
    end
