...
function calc_admittance_matrix(data::Dict{String,<:Any})
    if length(data["dcline"]) > 0
        @log_error("calc_admittance_matrix does not support data with dclines")
    end
    if length(data["switch"]) > 0
        @log_error("calc_admittance_matrix does not support data with switches")
    end
...
function calc_susceptance_matrix(data::Dict{String,<:Any})
    if length(data["dcline"]) > 0
        @log_error("calc_susceptance_matrix does not support data with dclines")
    end
    if length(data["switch"]) > 0
        @log_error("calc_susceptance_matrix does not support data with switches")
    end
...
function calc_susceptance_matrix_inv(data::Dict{String,<:Any})
    ...
    if !(ref_idx > 0 && ref_idx <= num_buses)
        @log_error("invalid ref_idx in calc_susceptance_matrix_inv")
    end
    S[ref_idx, :] .= 0.0
    S[:, ref_idx] .= 0.0
    ...
    if !LinearAlgebra.issuccess(F)
        @log_error("Failed factorization in calc_susceptance_matrix_inv")
    end
...
function calc_admittance_matrix_inv(am::AdmittanceMatrix{T}, ref_idx::Int) where {T}
    ...
    if !(ref_idx > 0 && ref_idx <= num_buses)
        @log_error("invalid ref_idx in calc_admittance_matrix_inv")
    end
...
function calc_bus_injection(data::Dict{String,<:Any})
    if length(data["dcline"]) > 0
        @log_error("calc_bus_injection does not support data with dclines")
    end
    if length(data["switch"]) > 0
        @log_error("calc_bus_injection does not support data with switches")
    end
