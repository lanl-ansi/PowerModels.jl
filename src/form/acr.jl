...
function constraint_power_balance(pm::GenericPowerModel{T}, n::Int, c::Int, i, bus_arcs, bus_arcs_dc, bus_gens, bus_storage, bus_pd_const, bus_qd_const, bus_pd_var, bus_qd_var, bus_gs_const, bus_bs_const, bus_gs_var, bus_bs_var) where T <: AbstractACRForm
    vr = var(pm, n, c, :vr, i)
    vi = var(pm, n, c, :vi, i)
    p  = var(pm, n, c, :p)
    q  = var(pm, n, c, :q)
    ...
    if length(bus_gs_var) != 0 || length(bus_bs_var) != 0
        @NLconstraint(pm.model,
            ...
            - sum(gs*fs[s]*(vr^2 + vi^2) for (s,gs) in bus_gs_var)
        )
        @NLconstraint(pm.model,
            ...
            + sum(bs*fs[s]*(vr^2 + vi^2) for (s,bs) in bus_bs_var)
        )
    else
        @constraint(pm.model,
            sum(p[a] for a in bus_arcs)
