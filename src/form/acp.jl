...
```
"""
function constraint_kcl_shunt(pm::GenericPowerModel{T}, n::Int, h::Int, i::Int, bus_arcs, bus_arcs_dc, bus_gens, bus_pd, bus_qd, bus_gs, bus_bs) where T <: AbstractACPForm
    vm = var(pm, n, h, :vm, i)
    p = var(pm, n, h, :p)
    q = var(pm, n, h, :q)
    pg = var(pm, n, h, :pg)
    ...
    con(pm, n, h, :kcl_p)[i] = @constraint(pm.model, sum(p[a] for a in bus_arcs) + sum(p_dc[a_dc] for a_dc in bus_arcs_dc) == sum(pg[g] for g in bus_gens) - sum(pd for pd in values(bus_pd)) - sum(gs for gs in values(bus_gs))*vm^2)
    con(pm, n, h, :kcl_q)[i] = @constraint(pm.model, sum(q[a] for a in bus_arcs) + sum(q_dc[a_dc] for a_dc in bus_arcs_dc) == sum(qg[g] for g in bus_gens) - sum(qd for qd in values(bus_qd)) + sum(bs for bs in values(bus_bs))*vm^2)
end
...
```
"""
function constraint_kcl_shunt_ne(pm::GenericPowerModel{T}, n::Int, h::Int, i, bus_arcs, bus_arcs_dc, bus_arcs_ne, bus_gens, bus_loads, bus_shunts, pd, qd, gs, bs) where T <: AbstractACPForm
    vm   = var(pm, n, h, :vm, i)
    p    = var(pm, n, h, :p)
    q    = var(pm, n, h, :q)
...
```
"""
function constraint_ohms_y_from(pm::GenericPowerModel{T}, n::Int, h::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr, tm, ta) where T <: AbstractACPForm
    p_fr  = var(pm, n, h,  :p, f_idx)
    q_fr  = var(pm, n, h,  :q, f_idx)
    vm_fr = var(pm, n, h, :vm, f_bus)
    vm_to = var(pm, n, h, :vm, t_bus)
...
```
"""
function constraint_ohms_y_to(pm::GenericPowerModel{T}, n::Int, h::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to, tm, ta) where T <: AbstractACPForm
    p_to  = var(pm, n, h,  :p, t_idx)
    q_to  = var(pm, n, h,  :q, t_idx)
    vm_fr = var(pm, n, h, :vm, f_bus)
    vm_to = var(pm, n, h, :vm, t_bus)
...
```
"""
function constraint_ohms_yt_from_on_off(pm::GenericPowerModel{T}, n::Int, h::Int, i, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm, vad_min, vad_max) where T <: AbstractACPForm
    p_fr  = var(pm, n, h,  :p, f_idx)
    q_fr  = var(pm, n, h,  :q, f_idx)
    vm_fr = var(pm, n, h, :vm, f_bus)
    vm_to = var(pm, n, h, :vm, t_bus)
...
```
"""
function constraint_ohms_yt_to_on_off(pm::GenericPowerModel{T}, n::Int, h::Int, i, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm, vad_min, vad_max) where T <: AbstractACPForm
    p_to  = var(pm, n, h,  :p, t_idx)
    q_to  = var(pm, n, h,  :q, t_idx)
    vm_fr = var(pm, n, h, :vm, f_bus)
    vm_to = var(pm, n, h, :vm, t_bus)
...
```
"""
function constraint_ohms_yt_from_ne(pm::GenericPowerModel{T}, n::Int, h::Int, i, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm, vad_min, vad_max) where T <: AbstractACPForm
...
```
"""
function constraint_ohms_yt_to_ne(pm::GenericPowerModel{T}, n::Int, h::Int, i, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm, vad_min, vad_max) where T <: AbstractACPForm
...
function constraint_voltage_angle_difference_on_off(pm::GenericPowerModel{T}, n::Int, h::Int, f_idx, angmin, angmax, vad_min, vad_max) where T <: AbstractACPForm
    i, f_bus, t_bus = f_idx
    va_fr = var(pm, n, h, :va, f_bus)
    va_to = var(pm, n, h, :va, t_bus)
    z = var(pm, n, h, :branch_z, i)
...
function constraint_voltage_angle_difference_ne(pm::GenericPowerModel{T}, n::Int, h::Int, f_idx, angmin, angmax, vad_min, vad_max) where T <: AbstractACPForm
    i, f_bus, t_bus = f_idx
    va_fr = var(pm, n, h, :va, f_bus)
    va_to = var(pm, n, h, :va, t_bus)
    z = var(pm, n, h, :branch_ne, i)
...
```
"""
function constraint_loss_lb(pm::GenericPowerModel{T}, n::Int, h::Int, f_bus, t_bus, f_idx, t_idx, g_fr, b_fr, g_to, b_to, tr) where T <: AbstractACPForm
    vm_fr = var(pm, n, h, :vm, f_bus)
    vm_to = var(pm, n, h, :vm, t_bus)
    p_fr = var(pm, n, h, :p, f_idx)
    q_fr = var(pm, n, h, :q, f_idx)
