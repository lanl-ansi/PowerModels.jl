export
    SOCWIPowerModel, SOCWIForm
""
abstract type AbstractWIForm <: AbstractPowerFormulation end

""
abstract type SOCWIForm <: AbstractWIForm end

""
const SOCWIPowerModel = GenericPowerModel{SOCWIForm}

"default SOC constructor"
SOCWIPowerModel(data::Dict{String,Any}; kwargs...) = GenericPowerModel(data, SOCWIForm; kwargs...)

""
function variable_current(pm::GenericPowerModel{T}, n::Int=pm.cnw; kwargs...) where T <: AbstractWIForm
    variable_series_current_magnitude_sqr(pm, n; kwargs...)
end

""
function variable_branch_flow(pm::GenericPowerModel{T}, n::Int=pm.cnw; kwargs...) where T <: AbstractWIForm
...
""
function variable_voltage(pm::GenericPowerModel{T}, n::Int=pm.cnw; kwargs...) where T <: AbstractWIForm
    variable_voltage_magnitude_sqr(pm, n; kwargs...)
end

"do nothing, this model does not have complex voltage constraints"
function constraint_voltage(pm::GenericPowerModel{T}, n::Int) where T <: AbstractWIForm
end
...
function constraint_ohms_yt_from(pm::GenericPowerModel{T}, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, c, tr, ti, tm) where T <: AbstractWIForm
    ...
    @constraint(pm.model, p_fr + p_to ==  g_sh_fr*(w_fr/tm^2) + r_s*i +  g_sh_to*w_to)
    @constraint(pm.model, q_fr + q_to == -b_sh_fr*(w_fr/tm^2) + x_s*i + -b_sh_to*w_to)
    ...
    #KVL over the line:
    @constraint(pm.model, w_to == (w_fr/tm^2) - 2(r_s*p_fr_s + x_s*q_fr_s) + (r_s^2 + x_s^2)*i)

    #convex constraint linking P, Q, W and I
    @constraint(pm.model, p_fr_s^2 +q_fr_s^2 <= (w_fr/tm^2)*i)
end
...
"""
function constraint_ohms_yt_to(pm::GenericPowerModel{T}, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, c, tr, ti, tm) where T <: AbstractWIForm
end


function constraint_voltage_angle_difference(pm::GenericPowerModel{T}, n::Int, i, f_bus, t_bus, angmin, angmax) where T <: AbstractWIForm
