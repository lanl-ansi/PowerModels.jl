...
function variable_dcline_current(pm::AbstractIVRModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true, kwargs...)
    ...
    var(pm, nw)[:p_dc] = p
    var(pm, nw)[:q_dc] = q
    report && _IM.sol_component_value_edge(pm, :ep, nw, :dcline, :pf, :pt, ref(pm, nw, :arcs_from_dc), ref(pm, nw, :arcs_to_dc), p)
    report && _IM.sol_component_value_edge(pm, :ep, nw, :dcline, :qf, :qt, ref(pm, nw, :arcs_from_dc), ref(pm, nw, :arcs_to_dc), q)
...
function objective_variable_pg_cost(pm::AbstractIVRModel; report::Bool=true)
    for (n, nw_ref) in nws(pm)
        ...
        report && _IM.sol_component_value(pm, :ep, n, :gen, :pg_cost, ids(pm, n, :gen), pg_cost)
...
function objective_variable_dc_cost(pm::AbstractIVRModel; report::Bool=true)
    for (n, nw_ref) in nws(pm)
        ...
        report && _IM.sol_component_value(pm, :ep, n, :dcline, :p_dc_cost, ids(pm, n, :dcline), dc_p_cost)
