...
# Currently adding all changes here, but they can be moved later

mutable struct SymmetricDCPLLPowerModel <: AbstractDCPLLModel @pm_fields end

"""
Creates Ohms constraints (yt post fix indicates that Y and T values are in rectangular form)
"""
function constraint_ohms_yt_to(pm::SymmetricDCPLLPowerModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm)
    p_to  = var(pm, n,  :p, t_idx)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)

    r = 0.5*g/(g^2 + b^2)
    p_to_no_loss = -b*(va_to - va_fr)
    JuMP.@constraint(pm.model, p_to >= p_to_no_loss + r*(p_to_no_loss^2))
end

""
function constraint_ohms_yt_from(pm::SymmetricDCPLLPowerModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
    p_fr  = var(pm, n,  :p, f_idx)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)

    r = 0.5*g/(g^2 + b^2)
    p_fr_no_loss = -b*(va_fr - va_to)
