...
""
function variable_shunt_factor(pm::AbstractWConvexModels; nw::Int=pm.cnw, cnd::Int=pm.ccnd, relax::Bool=false)
    if relax == true
    ...
    var(pm, nw, cnd)[:wz_shunt] = JuMP.@variable(pm.model,
        [i in ids(pm, nw, :shunt)], base_name="$(nw)_wz_shunt",
        lower_bound = 0,
        upper_bound = ref(pm, nw, :bus)[ref(pm, nw, :shunt, i)["shunt_bus"]]["vmax"]^2,
        start = comp_start_value(ref(pm, nw, :shunt, i), "wz_shunt_start", cnd, 1.001)
    )
end
...
""
function constraint_power_balance_ls(pm::AbstractWConvexModels, n::Int, c::Int, i::Int, bus_arcs, bus_arcs_dc, bus_arcs_sw, bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
    w    = var(pm, n, c, :w, i)
    p    = get(var(pm, n, c),    :p, Dict()); _check_var_keys(p, bus_arcs, "active power", "branch")
    q    = get(var(pm, n, c),    :q, Dict()); _check_var_keys(q, bus_arcs, "reactive power", "branch")
    pg   = get(var(pm, n, c),   :pg, Dict()); _check_var_keys(pg, bus_gens, "active power", "generator")
    ...
    wz_shunt = get(var(pm, n, c), :wz_shunt, Dict()); _check_var_keys(wz_shunt, keys(bus_gs), "voltage square power factor", "shunt")
