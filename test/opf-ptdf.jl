...
@testset "test ptdf-based dc opf" begin
    ...
    @testset "no support for zero reference buses" begin
        ...
        @test_throws(TESTLOG, ErrorException, PowerModels.run_opf_ptdf(data, DCPPowerModel, nlp_solver))
    end

    @testset "no support for multiple connected components" begin
        @test_throws(TESTLOG, ErrorException, PowerModels.run_opf_ptdf("../test/data/matpower/case6.m", DCPPowerModel, nlp_solver))
    end

    @testset "no support for dclines" begin
        @test_throws(TESTLOG, ErrorException, PowerModels.run_opf_ptdf("../test/data/matpower/case3.m", DCPPowerModel, nlp_solver))
    end

    @testset "no support for switches" begin
        @test_throws(TESTLOG, ErrorException, PowerModels.run_opf_ptdf("../test/data/matpower/case5_sw.m", DCPPowerModel, nlp_solver))
    end
