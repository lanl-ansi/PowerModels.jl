...
@testset "test ptdf-based dc opf" begin
    @testset "5-bus case, LP solver" begin
        ...
        result_pg = PowerModels._run_ptdf_opf(data, DCPPowerModel, cbc_solver)
    ...
    @testset "5-bus gap case" begin
        ...
        result_pg = PowerModels._run_ptdf_opf(data, DCPPowerModel, ipopt_solver)
    ...
    @testset "no support for zero reference buses" begin
        ...
        @test_throws(TESTLOG, ErrorException, PowerModels._run_ptdf_opf(data, DCPPowerModel, ipopt_solver))
    end

    @testset "no support for multiple connected components" begin
        @test_throws(TESTLOG, ErrorException, PowerModels._run_ptdf_opf("../test/data/matpower/case6.m", DCPPowerModel, ipopt_solver))
    end

    @testset "no support for dclines" begin
        @test_throws(TESTLOG, ErrorException, PowerModels._run_ptdf_opf("../test/data/matpower/case3.m", DCPPowerModel, ipopt_solver))
    end

    @testset "no support for switches" begin
        @test_throws(TESTLOG, ErrorException, PowerModels._run_ptdf_opf("../test/data/matpower/case5_sw.m", DCPPowerModel, ipopt_solver))
    end
