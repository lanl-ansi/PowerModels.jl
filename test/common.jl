...
function compute_basic_ac_pf!(data::Dict{String, Any})
    if !get(data, "basic_network", false)
        PowerModels.@_warn("compute_basic_ac_pf requires basic network data and given data may be incompatible. make_basic_network can be used to transform data into the appropriate form.")
    end
    ...
    if itr == itr_max
        PowerModels.@_warn("Max iteration limit")
        @assert false
    end
end
