...
@testset "test ac api" begin
    @testset "3-bus case" begin
        result = run_api_opf("../test/data/case3.json", APIACPPowerModel, ipopt_solver)
    ...
    @testset "5-bus pjm case" begin
        result = run_api_opf("../test/data/case5.json", APIACPPowerModel, ipopt_solver)
    ...
    @testset "30-bus ieee case" begin
        result = run_api_opf("../test/data/case30.json", APIACPPowerModel, ipopt_solver)
...
@testset "test ac sad" begin
    @testset "3-bus case" begin
        result = run_sad_opf("../test/data/case3.json", ACPPowerModel, ipopt_solver)

        @test result["status"] == :LocalOptimal
        @test isapprox(result["objective"], 0.3144; atol = 1e-2)
    end
    @testset "5-bus pjm case" begin
        result = run_sad_opf("../test/data/case5.json", ACPPowerModel, ipopt_solver)

        @test result["status"] == :LocalOptimal
        @test isapprox(result["objective"], 0.02233; atol = 1e-2)
    end
    @testset "30-bus ieee case" begin
        result = run_sad_opf("../test/data/case30.json", ACPPowerModel, ipopt_solver)
